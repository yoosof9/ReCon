...
class LogicalOREarlyStopping(Callback):
    r"""
    ...
    def __init__(
        ...
        self.wait_count = [0 for _ in self.monitor]
        ...
        self.best_score = [torch_inf if self.monitor_op == torch.lt else -torch_inf for _ in self.monitor]
    ...
    def _validate_condition_metric(self, logs: Dict[str, Tensor]) -> bool:
        for m in self.monitor:
            ...
            error_msg = (
                f"Early stopping conditioned on metric `{m}` which is not available."
    ...
    def _run_early_stopping_check(self, trainer: "pl.Trainer") -> None:
        ...
        if trainer.fast_dev_run or not self._validate_condition_metric(  # disable early_stopping with fast_dev_run
            ...
            return
        current_values = [logs[m].squeeze() for m in self.monitor]
        # current = logs[self.monitor].squeeze()
        should_stop, reason = self._evaluate_stopping_criteria(current_values)
    ...
    def _evaluate_stopping_criteria(self, current_values: list[Tensor]) -> Tuple[bool, Optional[str]]:
        # should_stop = False
        should_stop_list = []
        reason = None
        for idx, current in enumerate(current_values):
            ...
            if self.check_finite and not torch.isfinite(current):
                should_stop = True
                reason = (
                    f"Monitored metric {self.monitor[idx]} = {current} is not finite."
                    f" Previous best value was {self.best_score[idx]:.3f}. Signaling Trainer to stop."
                )
            elif self.stopping_threshold is not None and self.monitor_op(current, self.stopping_threshold):
                should_stop = True
                reason = (
                    "Stopping threshold reached:"
                    f" {self.monitor[idx]} = {current} {self.order_dict[self.mode]} {self.stopping_threshold}."
                ...
                reason = (
                    ...
                    f" {self.monitor[idx]} = {current} {self.order_dict[self.mode]} {self.divergence_threshold}."
                    " Signaling Trainer to stop."
                )
            elif self.monitor_op(current - self.min_delta, self.best_score[idx].to(current.device)):
                should_stop = False
                reason = self._improvement_message(current, idx)
                self.best_score[idx] = current
                self.wait_count[idx] = 0
            else:
                self.wait_count[idx] += 1
                if self.wait_count[idx] >= self.patience:
                    should_stop = True
                    reason = (
                        f"Monitored metric {self.monitor[idx]} did not improve in the last {self.wait_count[idx]} records."
                        f" Best score: {self.best_score[idx]:.3f}. Signaling Trainer to stop."
                    )
            should_stop_list.append(should_stop)
        should_stop = any(should_stop_list)
        if should_stop is False:
            reason = None
        return should_stop, reason

    def _improvement_message(self, current: Tensor, idx: int) -> str:
        """Formats a log message that informs the user about an improvement in the monitored score."""
        if torch.isfinite(self.best_score[idx]):
            msg = (
                f"Metric {self.monitor[idx]} improved by {abs(self.best_score[idx] - current):.3f} >="
                f" min_delta = {abs(self.min_delta)}. New best score: {current:.3f}"
            )
        else:
            msg = f"Metric {self.monitor[idx]} improved. New best score: {current:.3f}"
        return msg
