...
class OptimalTransportFcn(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, M, r=None, c=None, gamma=1.0, eps=1.0e-6, maxiters=1000, logspace=False, method='full'):
...
class OptimalTransportLayer(nn.Module):
    ...
    def __init__(self, gamma=1.0, eps=1.0e-6, maxiters=1000, logspace=False, method='full'):
...
if __name__ == '__main__':
    ...
    M = torch.randn((1, 5, 5), dtype=torch.double, requires_grad=True)
    M = torch.mul(M, M)
    D = M.detach().numpy().squeeze()
    # f = OptimalTransportFcn().apply
    # a = f(M)
    a = sinkhorn(M)
    
    # f = OptimalTransportFcn()
    # optimizer = optim.Adam(f.parameters(), lr=0.001)

    # for i in range(0, 100):
    # a = f.forward(M)
    b = torch.dot(a.flatten(),M.flatten())
    print(b)
    # loss = torch.sum(b)
    # loss.backward()
    # optimizer.step()
